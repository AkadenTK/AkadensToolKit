return { -- option: 1
    ...
    long_name = 'unity npc',
    npc_names = {
        warp = {"Igsli", "Urbiolaine", "Teldro-Kesdrodo", "Yonolala", "Nunaarl Bthtrogg"},
    },
    ...
    build_warp_packets = function(current_activity, zone, p, options)
        ...
        local accolades = string.unpack(p.params, 'i', 9)
        ...
        if not destination_unlocked then
            ...
            packet.zone_id = zone
            packet.menu_id = menu
            actions:add({packet=packet, description='cancel menu', message='Destination is not unlocked yet!'})
            return actions
        end
        ...
        if accolades < 100 then
            ...
            packet.zone_id = zone
            packet.menu_id = menu
            actions:add({packet=packet, description='cancel menu', message='Not enough accolades!'})
            return actions
        end
        ...
        actions:add({packet=packet, wait_packet=0x05C, delay=wiggle_value(options.simulated_response_time, options.simulated_response_variation), description='menu change'})
        ...
        actions:add({packet=packet, wait_packet=0x05C, expecting_zone=true, delay=wiggle_value(options.simulated_response_time, options.simulated_response_variation), description='send options and complete menu', message=popMessage})
