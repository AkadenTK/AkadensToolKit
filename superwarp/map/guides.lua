return { -- option: 1
    ...
    build_warp_packets = function(current_activity, zone, p, options)
        ...
        local gil = string.unpack(p.params, 'I', 9)
        local valor = string.unpack(p.params, 'H', 7)    

        local thrifty_transit = menu == 8501

        local unlock_bit_start = 12*8

        debug('guide is unlocked: '..tostring(has_bit(p.params, unlock_bit_start + destination.offset)))

        if not has_bit(p.params, unlock_bit_start + destination.offset) then
            ...
            packet.zone_id = zone
            packet.menu_id = menu
            actions:add({packet=packet, description='cancel menu', message='Destination Survival Guide is not unlocked yet!'})
            return actions
        end
        ...
        local use_tabs_at_survival_guides = not thrifty_transit and options.use_tabs_at_survival_guides
        ...
        if currency_stock < currency_minimum then
            ...
            packet.zone_id = zone
            packet.menu_id = menu
            actions:add({packet=packet, description='cancel menu', message='Not enough '..currency_name..'!'})
            return actions
        end
        ...
        packet.option_index = 7
        packet.option_index_2 = 0
        packet.not_exiting = true
        
        actions:add({packet=packet, delay=wiggle_value(options.simulated_response_time, options.simulated_response_variation), description='menu change'})
